...
const int32_t kAudioCaptureBufferSize = 40000; //old one had 80000
const int32_t i2s_bytes_to_read = 3200;

namespace {
int16_t g_audio_output_buffer[kMaxAudioSampleSize * 32]; //this might have to change
...
TfLiteStatus InitAudioRecording(tflite::ErrorReporter* error_reporter) {
  ...
  while (!g_latest_audio_timestamp) {
    vTaskDelay(1); // one tick delay to avoid watchdog may need to change to delay(1);
  }
  ESP_LOGI(TAG, "Audio Recording started");
  return kTfLiteOk;
}

TfLiteStatus GetAudioSamples1(tflite::ErrorReporter* error_reporter,
